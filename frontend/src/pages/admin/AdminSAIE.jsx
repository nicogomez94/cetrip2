...
import { SAIE_DEFAULTS } from '../../constants/publicPageDefaults';
...
function AdminSAIE() {
  ...
  const [highlightImage, setHighlightImage] = useState(SAIE_DEFAULTS.highlightImage);
  const [highlightText, setHighlightText] = useState(SAIE_DEFAULTS.highlightText);
  const [galleryImages, setGalleryImages] = useState(SAIE_DEFAULTS.gallery);
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const pageSections = await fetchSectionsByPage('saie');
      const highlightSection = pageSections.find((s) => s.slug === PAGE_SLUGS.saie.highlight);
      const gallerySection = pageSections.find((s) => s.slug === PAGE_SLUGS.saie.gallery);
      ...
      setHighlightImage(imgBlock?.imageUrl || SAIE_DEFAULTS.highlightImage);
      setHighlightText(txtBlock?.content || SAIE_DEFAULTS.highlightText);
      setGalleryImages(galleryBlocks.length > 0 ? galleryBlocks : SAIE_DEFAULTS.gallery);
    } catch {
      setError('No se pudo cargar el contenido de SAIE.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const highlightSectionId = await ensureSection({
        sections,
        page: 'saie',
        slug: PAGE_SLUGS.saie.highlight,
        title: 'SAIE – Destacado',
        description: 'Imagen y texto principal de la página SAIE',
        order: 1,
      });
      const gallerySectionId = await ensureSection({
        sections,
        page: 'saie',
        slug: PAGE_SLUGS.saie.gallery,
        title: 'SAIE – Galería',
        description: 'Galería de imágenes de la página SAIE',
        order: 2,
      });
      ...
      const message = err.response?.data?.message || 'No se pudo guardar SAIE.';
    ...
    <AdminLayout title="SAIE">
      ...
      <div className="admin-page">
        <div className="admin-form-card">
          <h3>Contenido de la página SAIE</h3>
